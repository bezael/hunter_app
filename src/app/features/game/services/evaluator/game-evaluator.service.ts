...
export class GameEvaluatorService {
  evaluateState(state: GameState): GameStatus {
    ...
    if (player.hasGold && player.position.x === startPosition.x && player.position.y === startPosition.y) {
  ...
  checkGoldCollection(state: GameState): GameState {
    ...
    if (!gold.collected && player.position.x === gold.position.x && player.position.y === gold.position.y) {
      return {
        ...state,
  ...
  checkWumpusEncounter(state: GameState): GameState {
    ...
    if (wumpus.alive && player.position.x === wumpus.position.x && player.position.y === wumpus.position.y) {
      return {
        ...state,
  ...
  checkWellFall(state: GameState): GameState {
    ...
    const isInWell = board.wells.some(well => well.x === player.position.x && well.y === player.position.y);
